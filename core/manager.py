...
from gbpservice.nfp.ahmed.core import executor as nfp_executor
from gbpservice.nfp.ahmed.core import log as nfp_logging
from gbpservice.nfp.ahmed.core import path as nfp_path
from gbpservice.nfp.ahmed.core import sequencer as nfp_sequencer
from gbpservice.nfp.ahmed.core import watchdog as nfp_watchdog
...
class NfpResourceManager(NfpProcessManager, NfpEventManager):
    ...
    def _scheduled_new_event(self, event):
		...
		print "Dispatching event - %s to worker" %(event.identify())
	...
	if event.desc.pid == os.getpid():
            return
        if not handler:
            handler = self.event_expired
        if event.lifetime != -1:
            print "Main Process - Starting watchdog..."
            wd = WATCHDOG(handler,
