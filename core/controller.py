...
from gbpservice.nfp.ahmed.core import cfg as nfp_cfg
from gbpservice.nfp.ahmed.core import common as nfp_common
from gbpservice.nfp.ahmed.core import context
from gbpservice.nfp.ahmed.core import event as nfp_event
from gbpservice.nfp.ahmed.core import launcher as nfp_launcher
from gbpservice.nfp.ahmed.core import log as nfp_logging
from gbpservice.nfp.ahmed.core import manager as nfp_manager
from gbpservice.nfp.ahmed.core import rpc as nfp_rpc
from gbpservice.nfp.ahmed.core import worker as nfp_worker
...
"""NFP Controller class mixin other nfp classes.
    ...
    in this class.
    Also, it mixes the other nfp.ahmed.core classes to complete
...
class NfpController(nfp_launcher.NfpLauncher, NfpService):
    ...
    def pipe_send(self, pipe, event, resending=False):
        ...
        if not resending:
            print "Stashingggg event - %s" %(event.identify())
...
class NfpGrpcServer(nfp_pb2_grpc.NfpServicer):
  ...
  def create_network_function(self, request, context):
    ...
    print "request namespace - %s" %(request.namespace)
    print "request data - %s" %(request.data)    
    topic = request.namespace
    controller.rpc_agents[topic]['agents'][0].manager.create_network_function(context, request.data)
    return nfp_pb2.NfpResponse(message='SUCCESS')
...
def main():
    ...
    print "Launched gRPC server - waiting"
